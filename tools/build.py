...
replace(old,old+'\n<button class="choice mode-card" data-setting="mode" data-value="doubles" type="button"><span class="mode-no">05 / 2 × 2</span><strong>四人双打</strong><small>上下协防 · 团队计分 · 四台设备</small></button>')
...
replace("if(g.isTimerMode())status+=' / TIME '+formatTime(g.match.remaining);", "if(g.isDoubles()){if(g.phase===Phase.PAUSED)status='PAUSED / HOST RESUMES';else if(g.phase===Phase.PLAYING&&g.serveSlot)status=g.serveSlot+' SERVE';if(g.localSeat())status+=' / YOU '+g.localSeat();}\n        if(g.isTimerMode())status+=' / TIME '+formatTime(g.match.remaining);")
replace("const paddles=[g.left,g.right];\n        for(let i=0;i<2;i++) {", "const paddles=g.getPaddles();\n        if(g.isDoubles()){const mid=this.wy(WORLD.height/2);for(let x=4;x<this.cols-4;x+=5)this.put(x,mid,'.');}\n        for(let i=0;i<paddles.length;i++) {")
replace("for(let y=y0;y<=y1;y++)this.put(x0,y,'#'.repeat(x1-x0+1));", "for(let y=y0;y<=y1;y++)this.put(x0,y,'#'.repeat(x1-x0+1));\n          if(g.isDoubles())this.put(p.side==='left'?x1+2:x0-5,Math.max(1,y0-1),(p.id===g.localSeat()?'>':' ')+p.id);")
...
replace("this.drawScore(g,p);this.drawLighting(g,p);this.drawPaddle(g,p,g.left,'left');this.drawPaddle(g,p,g.right,'right');", """this.drawScore(g,p);this.drawLighting(g,p);
        ...
        for(const d of g.getPaddles()){
          ...
          if(g.isDoubles()){
            c.save();const mine=d.id===g.localSeat();c.font=(mine?'700':'500')+' 10px ui-monospace,Consolas,monospace';c.fillStyle=mine?'#f6fdff':p[side];
            c.textAlign='center';c.globalAlpha=mine?.95:.70;
            c.fillText(d.id,side==='left'?16:944,d.y+d.height/2+3);
...
replace("if(game.isClient() && online.connected)online.sendRealtime", "if(game.isDoubles()){if(game.isClient())doubles.sendHost({t:'input',matchId:game.matchId,round:game.roundId,seq:++game.inputSeq,dir:0,target:null},true);return;}\n      if(game.isClient() && online.connected)online.sendRealtime")
...
replace("version:'4.1.0'", "version:'5.0.0'")
