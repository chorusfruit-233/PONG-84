...
check('Standalone entry metadata is present','apple-mobile-web-app-capable' in html and 'href="manifest.webmanifest"' in html)
check('ASCII entry differs only by its startup preference and manifest',ascii_html==html.replace("const bootRender=new URLSearchParams(location.search).get('render');","const bootRender='ascii'; // Independent zero-canvas startup entry.").replace('href="manifest.webmanifest"','href="ascii.webmanifest"'))
