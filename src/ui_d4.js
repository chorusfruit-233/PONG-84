    class DoublesUI extends UIController {
      ...
      updateD4(){
        ...
        for(const seat of D4.seats){const p=r.playerAt(seat),n=p?r.nodes.get(p.device):null,el=by('d4Seat'+seat),mine=g.isLocalSeat(seat),ai=r.isBot(p);
          ...
          this.setText('d4SeatState'+seat,!p?'等待玩家':p.pendingReturn?'AI 代打 · 下次发球归还':ai?'AI · 无增益':!n?.connected?'掉线保留':!n.visible?'后台':!n.synced?'正在同步':n.ready?'已准备':r.active?'比赛中':'未准备');
          this.setText('d4SeatMeta'+seat,p?.kind==='bot'?'375 移速上限 · 独立反应延迟':n?(n.id===r.hostId?'房主 · ':'')+n.id+(n.count===2?' · 同机玩家 '+(p.index+1):''):'点击选择');}
