    ...
    class DoublesRoom {
      ...
      receive(l,raw,ch){
        ...
        if(from===this.hostId){
          if(m.t==='state'&&(ch==='rt'||ch==='ctrl')){if(this.game?.isDoubles())this.game.applyD4(m.s);return;}
          if(ch==='ctrl'){
      ...
      sendState(s){if(this.role!=='host')return;const started=performance.now(),now=started;for(const [id,l] of this.links){const n=this.nodes.get(id);if(!n?.synced||!n.connected||!l.authed||!l.ctrl?.open&&!l.rt?.open)continue;
          const realtime=!!l.rt?.open,hz=!realtime?20:n.count===0?20:now<l.lowRateUntil?30:60;if(now-l.lastStateAt<1000/hz-2)continue;
          const live={...s,aux:{...s.aux,brains:undefined,tactics:undefined,teamwork:undefined}};
          if(this.sendTo(l,{t:'state',s:live},realtime)){l.lastStateAt=now;l.sentStates++;this.metrics.sendBacklog=Math.max(this.metrics.sendBacklog,l.rt?.dataChannel?.bufferedAmount||0);}}this.metrics.hostCompute=performance.now()-started;}
