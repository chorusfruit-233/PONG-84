...
**发布版本：6.2.0 · 团队协议：v4。** 所有参赛者和观众均需更新，并重新创建房间或邀请。新增本局战术记忆及迁移快照校验，不能与 v1/v2/v3 混用；原有单打玩法不受影响。
...
![前后阵型球场](screenshots/depth_court.png)
...
AI 的普通决策间隔为约 **45 毫秒**，临近接球或变轨期间约 **25 毫秒**；基础移动上限维持与真人相同的 **1100 逻辑单位 / 秒**，本次没有继续提高速度。球拍仍按物理步连续移动并遵守半区边界，不会直接跳到预测位置。2 个逻辑单位以内的误差停止移动，避免微小抖动。
...
AI 不再默认把球拍中心对准每一次来球。预判到达位置后，先计算自己能合法移动到的安全接球区间，再搜索最多 17 个候选击球位置。不同击球位置通过所有玩家共用的回弹规则自然产生直线、斜线、旋转和墙面反弹；规划过程不修改真实球状态。

每条候选球路都会检查对手的当前站位、从可见位置变化估计的移动方向、合法移速、球拍高度与功能球到期。在上下阵型的接缝处同时检查两名防守者，在前后阵型中同时计算前排和后排：绕过前排不等于已经得分。可以完成的突破优先于单纯打大角度；没有直接得分机会时，偏向压迫空当、反向调动及让对手更难回球。

为避免在最后一刻放弃进攻，球已经安全覆盖时会保持选定击球角度。无法安全调整的紧急来球则退回救球策略，不为了进攻故意漏球。发球时会选择更有压力的站位，但实际随机发球角度与真人规则一致。

**本版的“学习”是对局内的轻量战术反馈，不是训练好的神经网络、强化学习模型或自我进化服务。** 两队分别记录六类出球方式的反馈：得分最高，破盾次之，迫使对手用球拍边缘勉强回球是较弱的正反馈，被轻松回球则降低偏好。结合上次落点与对手偏位情况，减少反复使用无效的固定球路。

战术记忆在同一局的各回合之间保留，随已提交的房主迁移检查点恢复；新开一局重置。不上传云端，也不在浏览器外持续训练。对手输入、其他 AI 的内部目标、尚未发生的随机变轨或未来道具均不提供给进攻规划器。

保持默认 **“四人双打 → AI 补位 · 主动进攻 / 对等增益”** 即自动生效，无需下载模型或额外服务。单人人机模式保持原有代码与电脑无增益规则。
...
本版使用当前房主统一计算、备份状态、心跳、多数确认与任期隔离。新房主从最近**已提交**的完整检查点恢复，可能回退短暂的最近状态；恢复包括比分、球、球拍、发球轮次、道具计时和本局 AI 战术反馈。原房主掉线席位按 AI 设置处理，确认后倒计时继续。检查点被网络积压拖延时，不能保证回退固定少于某个时间。
...
**协议测试明确使用内存消息转发替身，不是实际 WebRTC 连通测试。** 本次 285 项功能断言包含 240 组来球轨迹、100 组回弹响应和多组本地模拟对局（样例数量不额外算作断言数），不应将这些结果解释为公网实战验收。详见 [TEST_REPORT.md](TEST_REPORT.md) 与 [PERFORMANCE.md](PERFORMANCE.md)。
