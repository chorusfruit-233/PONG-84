...
**发布版本：6.0.0 · 团队协议：v2。** 所有参赛者和观众应使用同一发布版本。新团队房间不兼容旧双打协议 v1。
...
AI 使用独立速度上限、反应间隔和瞄准误差，不获得长拍、临时移速、攻击球加速、小球、旋转、变轨等强化；AI 回球回到基础速度约束。长拍只选择正在由真人控制的球拍。

**含有 AI 的整支队伍不享受共享护盾，也不获得来球减速或大球带来的防守便利。** 这是为了避免团队道具间接给 AI 增益；与 AI 同队的真人也受这两项团队限制影响。恢复为两名真人后再按正常规则计算，不返还已经撤销的护盾。全局仍只有一套功能球计时器，不因连接人数增加而重复生成。
...
**协议测试明确使用内存消息转发替身，不是实际 WebRTC 连通测试。** 不应将 178 项自动检查解释为公网实战验收。详见 [TEST_REPORT.md](TEST_REPORT.md) 与 [PERFORMANCE.md](PERFORMANCE.md)。
