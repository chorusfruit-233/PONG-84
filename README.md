...
| 模式 | 内容 |
| --- | --- |
| 人机对战 | 简单 / 普通 / 困难 / 地狱 / 半神；旧四档电脑无增益，半神使用对等增益 |
| 本地双人 | 两人在同一设备上左右对战 |
| 双人计时 | 按指定比赛时长结算 |
...
选择 **人机对战 → 半神 → 开始游戏**。图形与 ASCII 入口均可使用，难度会保存到本地设置。

- **精确防守：** 使用实际单人物理固定步长预测落点，覆盖球半径、墙面反弹、旋转、变轨、功能球出现及效果到期。没有观察延迟、反应等待、瞄准误差、死区、随机失误或转向惩罚。
- **强攻选点：** 复用四人 AI 的候选击球偏移、空当与反向调动评分，以及前三条球路的回复推演；进攻候选使用单人真实回球规则评估。每一步都重新保证防守可达，不能为了进攻放弃本可接住的球。
- **速度仍有上限：** 默认与真人键盘球拍一致，为 **1100 逻辑单位/秒**。不瞬移，不改碰撞，不拦截已经超出可达范围的球。可用快球和大范围调动得分。
- **对等增益：** 基础拍高 104，与真人一致；可获得长拍、连得分护盾、加速、旋转与变轨。简单至地狱仍保持原有无增益规则，四人 AI 仍保留有限反应与移动规则。

半神局为功能球和变轨使用独立的随机序列；预测复制序列状态，不消耗或改变实际比赛的后续随机结果。发球仍使用原来的随机角度，不能预知玩家下一次输入。进攻搜索每 50 毫秒更新，防守每个物理步更新，不受该搜索间隔限制。
...
**以下新规则仅用于“四人双打 / 团队模式”，不要求四席都是真人。人机的简单至地狱仍保留电脑无增益限制；新“半神”独立采用对等增益。**
...
**降低的是控制器能力，不是增益资格或倍率。** 长拍、团队护盾、球速、大小球、旋转和变轨继续与真人共用规则。原有四档单人人机难度及其“电脑无增益”约束不变；新半神不受此约束。本版未增加四人 AI 难度选项。
...
支持时，“全屏”使用浏览器原生全屏；iPhone 等不支持的环境显示“沉浸模式”，隐藏页面非必要区域、占满可用视口。原生全屏请求被拒绝也会降级为沉浸模式，并给出普通提示。浏览器工具栏可能仍然显示，不会强制隐藏系统界面。两种模式均保留左上角退出按钮，支持 Esc / F 返回；退出沉浸模式恢复原页面滚动位置。

球场保持 **16:9 等比居中**，为刘海和底部手势条预留安全区；手机旋转和工具栏收放时重新适配。触控按钮至少 44 像素，游戏触控区避免拖动页面。手机竖屏轻提示横屏，不强制锁定方向。

**iPhone / iPad 主屏幕体验：** 在 Safari 打开已部署的页面，选择“分享 → 添加到主屏幕”，如有“作为网页 App 打开”请开启。从图标启动会使用独立窗口并默认进入沉浸布局；普通入口和 ASCII 入口各自保持启动方式。部署时同时保留 `manifest.webmanifest`、`ascii.webmanifest` 和 `assets/`。单独 HTML 文件仍可游玩，但不保证主屏幕安装体验。本次未加入离线缓存，首次及后续资源加载仍依赖可用网络。
